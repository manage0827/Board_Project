...
<script>
		...
		// 댓글 목록
		    function listReply(){
		    	 ...
		    	 $.ajax({
						type:'get',
						url:'reply/replyList?bno=${dto.bno}&curPage=${curPage}',
						dataType : 'JSON',
						success: function(result){
							...
							for(i=0; i < result.length; i++){
								html.push('<tr>');
								html.push('<td>' + result[i].userName);
								html.push("("+changeDate(result[i].regdate)+")<br>");
								html.push(result[i].replytext + '</td>');
								html.push('</tr>');
